...
window.LP_PWA = (function () {
  ...
  async function saveBook(dir, chapters, onProgress) {
    ...
    if (await put(`${dir}/film/film.js`)) {
      for (const f of ['', 'index.html', '3d.html', 'film.css', 'script.js', 'cast.js', 'shots.js', 'frames.js', 'film3d.js', 'scenes3d.js', 'vendor/three.min.js']) await put(`${dir}/film/${f}`);
      // the film's own pictures (film/frames.js → images/film/<id>.jpg)
      try {
        const r = await cache.match(`${dir}/film/frames.js`);
        const ids = r ? [...(await r.text()).matchAll(/id: '([\w-]+)'/g)].map(m => m[1]) : [];
        for (const id of ids) await put(`${dir}/images/film/${id}.jpg`);
      } catch (e) { /* no film pictures */ }
    }
