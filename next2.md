...
1. **공개 방식 결정**: 푸시하면 사이트에 바로 반영됩니다. 모션을 기본으로 켤지, `?motion=1`로만 켜지게 해서 먼저 원격에서 볼지 정해야 합니다(`motion.js`의 `off` 조건 한 줄).
2. **커밋·푸시**: 게임 관련 변경은 빼고 영화 관련 파일만 넣습니다.
   `film/`, `images/layers/`, `images/README.md`, `README.md`, `js/pwa.js`, `sw.js`, `tools/film-*.py`, `tools/test.js`, `motion.md`, `next2.md`
3. **5단계(숨쉬기·목도리)와 7단계(해·별 층 애니메이션, 짧은 영상 반복)**: 아직 손대지 않았습니다. 할지 사용자 확인이 필요합니다.
